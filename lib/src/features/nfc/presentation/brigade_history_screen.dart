...
class BrigadeHistoryScreen extends StatefulWidget {
  ...
  @visibleForTesting
  static List<PatientFullRecord>? debugPatients;
...
class _BrigadeHistoryScreenState extends State<BrigadeHistoryScreen> {
  ...
  void initState() {
    super.initState();
    if (BrigadeHistoryScreen.debugPatients != null) {
      _patients.addAll(BrigadeHistoryScreen.debugPatients!);
    }
    _simulateSync();
  }
                          ...
                          child: _patients.isEmpty
                              ...
                              : ListView.separated(
                                  ...
                                  itemBuilder: (_, i) {
                                    final p = _patients[i];
                                    return _PatientRow(
                                      name: p.patientInfo.fullName,
                                      date: p.patientInfo.dob,
                                      status: _status,
                                      s: s,
                                    );
                                  },
  ...
  Color get _headerColor => switch (_status) {
    _SyncStatus.pending => const Color(0xFFD4A017),
    _SyncStatus.synchronizing => AppColors.secondary,
    _SyncStatus.synchronized => const Color(0xFF2E7D32),
  };
}
...
class _PatientRow extends StatelessWidget {
  ...
  IconData get _icon => switch (status) {
    _SyncStatus.pending => Icons.cloud_upload_outlined,
    _SyncStatus.synchronizing => Icons.sync,
    _SyncStatus.synchronized => Icons.cloud_done,
  };
  Color get _color => switch (status) {
    _SyncStatus.pending => const Color(0xFFD4A017),
    _SyncStatus.synchronizing => AppColors.primary,
    _SyncStatus.synchronized => const Color(0xFF2E7D32),
  };
  String get _label => switch (status) {
    _SyncStatus.pending => s.pending,
    _SyncStatus.synchronizing => s.synchronizing,
    _SyncStatus.synchronized => s.synchronized,
  };
}
