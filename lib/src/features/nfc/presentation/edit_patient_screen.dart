// ============================================================
// This file bundles i18n migrations for all remaining screens.
// Each class goes into its own file in the real project.
// ============================================================
...
class _EditPatientScreenState extends State<EditPatientScreen> {
  ...
  late final TextEditingController _weightCtrl, _heightCtrl, _streetCtrl, _cityCtrl, _stateCtrl;
  late String _nationalityCode;

  static const _nationCodes = {'COL': 'Colombia', 'VEN': 'Venezuela', 'ECU': 'Ecuador', 'PER': 'Perú'};

  @override
  void initState() {
    super.initState();
    final info = widget.patient.patientInfo;
    _fullName = info.fullName; _dob = info.dob;
    _bloodType = info.bloodType ?? 'N/A';
    _documentInfo = '${info.identification.documentType} ${info.identification.documentNumber}';
    ...
    _nationalityCode = _nationCodes.containsKey(info.nationalityCode) ? info.nationalityCode : 'COL';
    _biologicalSex = info.biologicalSex;
  }

  @override
  void dispose() { _weightCtrl.dispose(); _heightCtrl.dispose(); _streetCtrl.dispose(); _cityCtrl.dispose(); _stateCtrl.dispose(); super.dispose(); }
        ...
        Column(children: [
          ...
          Expanded(child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(s.fieldsProtected, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
              ...
              const SizedBox(height: 10),
              Row(children: [
                Expanded(child: _ro(s.dateOfBirth, _dob, Icons.calendar_today)),
                const SizedBox(width: 10),
                Expanded(child: _ro(s.gender, sexLabels[_biologicalSex] ?? _biologicalSex, Icons.wc)),
              ]),
              const SizedBox(height: 10),
              ...
              _sec(s.editableInfo),
              const SizedBox(height: 12),
              _dd(s.nationality, _nationalityCode, _nationCodes, (v) { if (v != null) setState(() => _nationalityCode = v); }),
              const SizedBox(height: 12),
              _tf(s.weight, _weightCtrl, icon: Icons.monitor_weight, keyboard: TextInputType.number),
              const SizedBox(height: 12),
              _tf(s.height, _heightCtrl, icon: Icons.open_in_full, keyboard: TextInputType.number),
              const SizedBox(height: 20),
              _sec(s.address),
              const SizedBox(height: 12),
  ...
  Widget _ro(String label, String val, IconData icon) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(label, style: const TextStyle(fontSize: 13)),
    const SizedBox(height: 4),
    Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
      decoration: BoxDecoration(color: const Color(0xFFE8E8E8), borderRadius: BorderRadius.circular(10)),
      child: Row(children: [Icon(icon, size: 18, color: AppColors.disabled), const SizedBox(width: 8),
        Expanded(child: Text(val, style: const TextStyle(fontSize: 14, color: AppColors.textSecondary))),
        const Icon(Icons.lock_outline, size: 14, color: AppColors.disabled)])),
  ]);
  Widget _tf(String label, TextEditingController c, {IconData? icon, TextInputType keyboard = TextInputType.text}) =>
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(fontSize: 13)), const SizedBox(height: 4),
      TextField(controller: c, keyboardType: keyboard, style: const TextStyle(fontSize: 14),
  ...
  Widget _dd(String label, String val, Map<String, String> opts, ValueChanged<String?> cb) =>
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(fontSize: 13)), const SizedBox(height: 4),
  ...
  Widget _btns(BuildContext context, AppStrings s) => Row(children: [
