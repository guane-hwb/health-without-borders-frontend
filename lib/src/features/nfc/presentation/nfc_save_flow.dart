...
class _NfcSaveFlowSheetState extends State<_NfcSaveFlowSheet> {
  ...
  Future<void> _start() async {
    ...
    if (widget.onSync != null) {
      try {
        final bool ok = await widget.onSync!();
        if (!mounted) return;
        if (ok) {
          setState(() => _state = _SS.success);
        } else {
          setState(() {
            _state = _SS.error;
            _err =
      ...
      if (mounted) setState(() => _state = _SS.success);
    }
  }
  ...
  Widget _buildPut(AppStrings s) => Column(
    mainAxisSize: MainAxisSize.min,
    children: [
      Text(
        s.putOnWristband,
  ...
  Widget _buildRegistering(AppStrings s) => Column(
    mainAxisSize: MainAxisSize.min,
    children: [
      Text(
        s.syncingServer,
  ...
  Widget _buildSuccess(AppStrings s) => Column(
    mainAxisSize: MainAxisSize.min,
    children: [
      const Icon(Icons.check_circle, size: 80, color: AppColors.success),
      const SizedBox(height: 16),
      Text(
        s.successRegistration,
        style: const TextStyle(
          fontSize: 20,
          fontWeight: FontWeight.w600,
          color: AppColors.primary,
  ...
  Widget _buildError(AppStrings s) => Column(
    mainAxisSize: MainAxisSize.min,
    children: [
