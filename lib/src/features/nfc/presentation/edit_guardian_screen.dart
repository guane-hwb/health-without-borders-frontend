...
class _EditGuardianScreenState extends State<EditGuardianScreen> {
  late final TextEditingController _nameCtrl, _docNumberCtrl, _addressCtrl, _contactCtrl;
  String _docType = 'CC';
  String _country = 'COL';
  ...
  @override
  void dispose() { _nameCtrl.dispose(); _docNumberCtrl.dispose(); _addressCtrl.dispose(); _contactCtrl.dispose(); super.dispose(); }

  @override
  Widget build(BuildContext context) {
    final s = AppStrings.of(context);
    const docTypes = {'CC': 'Cédula de Ciudadanía', 'PA': 'Pasaporte', 'CE': 'Cédula Extranjería'};
        ...
        Column(children: [
          ...
          Expanded(child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(s.guardianSection, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.primary)),
              const SizedBox(height: 16),
              _tf(s.name, _nameCtrl, icon: Icons.person),
              const SizedBox(height: 14),
              _dd(s.documentType, _docType, docTypes, (v) { if (v != null) setState(() => _docType = v); }),
              const SizedBox(height: 14),
              _tf(s.documentNumber, _docNumberCtrl, icon: Icons.badge),
              const SizedBox(height: 14),
              _dd(s.nationality, _country, countries, (v) { if (v != null) setState(() => _country = v); }),
              const SizedBox(height: 14),
